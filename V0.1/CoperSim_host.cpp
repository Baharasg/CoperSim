...
double compute_latency = 0;
double memory_latency = 0;
float mem_comp_ratio = 0;
double transferred = 0;
double useful_transferred = 0;
float bw_util = 0;
double total_blocks = 0;
double T_dot_total = 0;
double T_mem_total = 0;
double T_BRAM_total = 0;
double T_decoprs_total = 0;
double T_compute_total = 0;
double total_latency = 0;
float total_mem_comp_ratio = 0;
float avg_mem_comp_ratio = 0;
double total_transferred = 0;
double total_useful_transferred = 0;
...
if (argv[1] == std::string("dense"))
{
	...
	while(getline(dense_file, dense_line))
	{
		...
		printf("%f \t", compute_latency);  // T_compute
 		T_compute_total = T_compute_total + compute_latency;
 		printf("%f \t", max(compute_latency,memory_latency)); // T_max
 		...
 		printf("%f \t", transferred); // data
 		printf("%f \t", useful_transferred); // usf_data
...
printf("final_results\t %f\t %f\t %f\t %f\t %f\t %f\t %f\t %1.2f\t %f\t %f\t %1.2f\n",
